...
class TestMovePiece(unittest.TestCase):
    ...
    def test_valid_cell(self):
        ...
        self.assertEqual(self.game.board.V[1].status,self.game.current_player().turn)

    def test_opponent_cell(self):
        self.game.board.select_piece(self.game.current_player(),3)
        self.game.board.move_piece(self.game.current_player(),4)
    ...
    def test_player_cell(self):
        ...
        self.game.board.move_piece(self.game.current_player(),11)
        self.assertEqual(self.game.board.V[6].status,self.game.current_player().turn)

    def test_nonAdj_cell(self):
        self.game.board.select_piece(self.game.current_player(),0)
        self.game.board.move_piece(self.game.current_player(),21)
        self.assertEqual(self.game.board.V[0].status,self.game.current_player().turn)
